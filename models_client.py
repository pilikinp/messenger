...
class Client():
    ...
    def connect_guest(self, property):
        ...
        #
        # time.sleep(5)
        # if self.data['response'] == '200':
        #     print(self.data['alert'])
        #     self.rep = Repository(self.username)
        # elif self.data['response'] == '201':
        #     print(self.data['alert'])
        #     print('Вы зарегистрировались под ником {}'.format(self.username))
        #     self.rep = Repository(self.username)
        # elif self.data['response'] == '409':
        #     print(self.data['alert'])
        #     sys.exit()
        # else:
        #     print('Возникла ошибка {} обратитесь к справке или напишите в поддержку'.format(self.data['response']))
        #     sys.exit()
    ...
    def msg_to(self, *args):
        sock = args[0]
        self._send_message(sock,'msg_to')
    ...
    def run(self):
        with self.socket as sock:
            ...
            if command.upper() == '1':
                ...
                sys.exit()
            while True:
                t1 = threading.Thread(target=self._get_message, args= (sock,))
